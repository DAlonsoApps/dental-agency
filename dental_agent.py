...
# ---------------------------------------------------------------------------
# Conversation memory (checkpointer) — a real Postgres database is the
# source of truth for conversation history when DATABASE_URL is set,
# otherwise an in-memory stand-in (with a loud warning), same fallback
# pattern as _build_calendar_client() above.
#
# Why this matters: LangGraph's MemorySaver keeps every conversation's
# message history in this process's own RAM only. That's invisible during
# local testing (the process just keeps running), but on a host like
# Render's free tier -- which has NO persistent disk and restarts the
# process on every redeploy, and also puts it to sleep after ~15 minutes of
# no traffic and cold-starts it on the next request -- that RAM is wiped
# constantly. A patient could be mid-conversation, go quiet for 20 minutes,
# come back, and the agent would have completely forgotten them. Backing
# this with Postgres (via LangGraph's PostgresSaver checkpointer) means
# conversation history survives all of that, exactly like the Google
# Calendar switch made bookings themselves survive it.
#
# --- Setting up a free Postgres database (Neon) ---
#   1. Go to https://neon.tech, sign up (free, no credit card required for
#      the free tier), and create a new project.
#   2. On the project dashboard, copy the "Connection string" shown for the
#      default branch/database. It looks like:
#        postgresql://<user>:<password>@<host>.neon.tech/<dbname>?sslmode=require
#   3. Set that as the DATABASE_URL environment variable (in your local
#      .env file for local testing, and in Render's dashboard -> your
#      service -> Environment tab, for the deployed backend).
#   4. That's it -- no manual table creation needed. The first time this
#      app starts up with DATABASE_URL set, PostgresSaver.setup() below
#      creates the checkpoint tables automatically (and does nothing if
#      they already exist, so it's safe to leave that call in permanently).
#
# Neon's free tier scales to zero when idle (no cost, and nothing to
# forget to shut down), which pairs naturally with Render's free tier also
# sleeping when idle.
# ---------------------------------------------------------------------------
...
def _build_checkpointer():
    ...
    if pool is not None:
        ...
        return checkpointer
    print(
        "WARNING: DATABASE_URL not set -- using an in-memory MemorySaver for "
        "conversation history instead of Postgres. Conversation memory will "
        "NOT survive a restart/redeploy until this is configured. See "
        "dental_agent.py's _build_checkpointer() for setup instructions.",
        file=sys.stderr,
    )
    return MemorySaver()
