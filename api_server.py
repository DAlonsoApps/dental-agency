...
NOTE on scaling beyond a single process: MemorySaver keeps conversation
history in this process's RAM only. That's fine for one server instance,
but if you ever run multiple instances behind a load balancer (for
reliability or throughput), a patient's follow-up message could land on a
different instance that has no memory of their earlier turns. At that
point, swap MemorySaver for a shared backend (LangGraph has Postgres/Redis
checkpointer implementations) -- not needed for a single-instance deployment.
"""
